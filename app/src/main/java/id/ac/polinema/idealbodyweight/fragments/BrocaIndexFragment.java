...
public class BrocaIndexFragment extends Fragment {
    ...
    private menuFragments.OnFragmentInteractionListener mListener;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final RadioGroup gnder = view.findViewById(R.id.gnder);
        final EditText height = view.findViewById(R.id.HeighText);

        Button hitung = view.findViewById(R.id.hitung);
        hitung.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (mListener != null) {
                    String heightString = height.getText().toString();
                    int checkedId = gnder.getCheckedRadioButtonId();
                    if ((checkedId != -1) && !TextUtils.isEmpty(heightString)) {
                        int height = Integer.parseInt(heightString);
                        int gender = (checkedId == R.id.lakilakiButton) ? BrocaIndex.MALE : BrocaIndex.FEMALE;
    ...
    public void onAttach(Context context) {
        ...
        if (context instanceof OnFragmentInteractionListener) {
            mListener = (menuFragments.OnFragmentInteractionListener) context;
        } else {
